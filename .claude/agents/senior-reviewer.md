...
10. **CLAUDE.md / change-control compliance.** Feature branch used (never commit directly to main)? Quality gates run (pytest cov≥80, ruff, mypy strict, bandit, import-boundary)? Mandatory doc/ADR updates performed per the matrix? Decision logged in `ground-truth/decisions.jsonl`? Non-obvious bug → a `debug-verbose` case study added? If the PR is up: is it a **draft** carrying a falsifiable **manual-testing checklist** (step → expected outcome)? An opened-non-draft PR, or a draft missing the manual-testing checklist, is P1 — the user's manual test is the sovereign gate and the checklist is how they run it.
