...
6. **Tests.** Coverage of risky paths, not happy paths only. Integration test through the mocked annotator for any pipeline change. Tests that pin behaviour vs assert implementation trivia. New code paths unexercised. Fragile timing/constant dependencies.
7. **Documentation accuracy.** Where the change touches behaviour described in `CLAUDE.md`, arc42 chapters under `Documentation/`, ADRs, or the model card/datasheet — do the docs still match? Documentation drift is debt; flag it. A new dependency / calibration-or-conformal change / LLM-boundary change without an ADR is P1.
...
10. **CLAUDE.md compliance.** Feature branch used (never commit directly to main)? Quality gates run (pytest cov≥80, ruff, mypy strict, bandit, import-boundary)? Mandatory doc/ADR updates performed? Decision logged in `ground-truth/decisions.jsonl`? Non-obvious bug → a `debug-verbose` case study added?
