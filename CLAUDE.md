...
**Current phase:** EPIC 7 — Eval harness + honesty artifacts (Module 03c): `eval/{schemas,metrics,scoring,runner}.py` + `scripts/run_eval.py` re-score the committed models on the reproduced homology split (per-ST + unseen-lineage + selective pair + reliability), cross-checked bit-for-bit against each committed `metrics.json` (all 5 drugs `committed_match=true`, max delta 0.0); real `models/eval_summary.json` + top-level `MODEL_CARD.md`/`DATASHEET.md` from real numbers (gentamicin generalizes on unseen ST307, AUROC 0.882; the other 4 unseen-lineage folds are single-class/thin, stated honestly); ADR-0024 + §11.4 wrong-but-valid-split lesson. All local gates green (404 passed, cov 96.9%). Draft PR pending user manual test. Prior phase: EPIC 6 — Demo surface (Module 03b): `service.py` in-process orchestrator (`analyze_genome`, ADR-0022) that both the FastAPI backend (`api/`) and the Streamlit UI (`ui/`) call. All local gates green (372 passed, cov 96.9%); `main` (PR #42, EPIC 4+5) merged into the branch; draft PR pending user manual test. Prior phase: EPIC 3 — Predictor (Module 02). PR-B (predict/registry/tracking, issues #21/#22) implemented on branch `feat/epic3-predict-registry`: `predictor/{conformal,predict,errors,model_registry,experiment_tracking}`, `scripts/train_predictor.py`, ADR-0014, full PR-B test suite (conformal/registry/predict/tracking + PR-B e2e). All local gates green (219 passed, cov 96%). **Real training run done** on a 130-genome/67-ST BV-BRC subset (Docker AMRFinderPlus → feature matrix → train_predictor → 5/5 drugs trained, MLflow-tracked); real metrics in `models/results_summary.json` + per-drug `model_card.md` (honest: gentamicin AUROC 0.875 / PR-AUC 0.76; beta-lactam gate-negative residual thin at n=130, `conformal_guarantee_available=false` for 4/5 — surfaced, not hidden).
**Completed:** EPIC 0 scaffolding; EPIC 1 (PR #37); EPIC 2 (PR #38); EPIC 3 PR-A (split/gate/LR+calibration + features/ + HTTPS fetch + batch feature-matrix builder, ADRs 0015–0018; draft PR pending manual test) and PR-B (conformal + predict + typed-compat registry + MLflow tracking + train orchestration + real run, ADR-0014). EPIC 4+5 (report/llm/kb, ADRs 0019–0021) merged as PR #42. EPIC 6 (api/ui/service, ADR-0022) implemented this session — draft PR pending user manual test. EPIC 7 (eval harness + MODEL_CARD + DATASHEET, ADR-0024) implemented this session on `feat/epic7-eval-cards` — draft PR pending user manual test.
**Next up:** senior review → draft PR-B → user manual test → merge (PR-A then PR-B).
**Parallel package (Module 03a):** EPIC 4 + EPIC 5 implemented on `feat/epic4-5-report-and-llm` (forked off `main`, schema-decoupled from the EPIC 3 predict-registry session): `report/{inputs,evidence,builder,narrative,nl_schemas,narrator,reviewer,pipeline}.py`, `llm/` (client + MockLLMClient + OpenAI backend), `kb/` (BM25+embedding+RRF evidence RAG, offline-safe). ADRs 0019–0020. All local gates green (296 passed, cov 97.1%). Draft PR pending manual test; real OpenAI path is mock-only in CI. EPIC 4+5 merged as PR #42.
**Issue #45 (defense-in-depth hardening of `report/`):** on branch `feat/issue45-reviewer-hardening` (off `main`) — reviewer per-drug number binding + intra-line `_PERCENT_RE` + published-string tripwire (A/B) and word-boundary disclaimer-dedup (C), ADR-0023. All local gates green (349 passed, cov 95.5%). Draft PR pending manual test.
...
- **Symptom:** a deterministic guard passes on the text it validates, yet the string actually published to the clinician can contain a fabricated confidence (`NN%`) the guard never saw; separately, a per-drug narrative can quote a confidence belonging to a *different* drug, and a legitimate distinct caveat is silently dropped. **Root cause:** the guard checked a PROXY, not the published artifact at the owning scope — the reviewer validated a differently-ordered serialization than `_flatten` publishes (`_PERCENT_RE`'s `\s*` spanned a newline), accepted numbers by *global* report membership rather than per-drug, and de-duplicated the disclaimer with a `"confirm"` substring that also matched `"confirmed"`. **Prevention:** validate the exact published string (a `published_percents_grounded` tripwire fails `narrate_report` closed on any ungrounded published percent), bind each `N%` to that drug's own rendered confidence and each number to that drug's row/cited-chunk, anchor `_PERCENT_RE` intra-line (`[^\S\r\n]*%`), and match the disclaimer with word boundaries + its "result" skeleton (`\bconfirm\b`, not substring `confirm`). Same class as the four-round narrative-leak lesson: guard the artifact you ship, at the scope that owns the datum. See §11.4 (issue #45) and ADR-0023.
- **Symptom:** an evaluation harness that re-scores a persisted model on a *reproduced* grouped split can silently report leaked metrics even though the split's own no-leakage check passes. **Root cause:** the split's seed/n_splits/backend are not persisted with the model, so eval assumes the training defaults; a wrong assumed seed yields a *different-but-internally-disjoint* split whose test fold overlaps the model's real training genomes — `no_leakage_check` only asserts the reproduced sets are mutually disjoint, not that they match what the `.joblib` actually saw. **Prevention:** cross-check the re-derived committed metric sets against the committed `metrics.json` field-for-field (`eval.runner.ReproCheck.committed_match`) and fail closed on divergence; validate the guard by *tampering* the committed artifact, because a perfectly-separable synthetic model scores identically on every fold and so a wrong-seed alone cannot demonstrate the leak. See §11.4 (EPIC 7 #29) and ADR-0024.
