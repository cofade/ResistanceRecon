...
## 10.6 Realization status (EPIC 7 — Evaluation harness + honesty artifacts)

EPIC 7 completes the measurement side of the *Safety & honesty* and *Correctness* rows — real held-out metrics at all three granularities, plus the model card and datasheet from real numbers:

- **Generalize beyond clones (measured)** — `eval/runner.py` re-scores the committed models on the reproduced homology split ([ADR-0024](../09-architecture-decisions/ADR-0024-eval-harness-rescore-reproduced-split.md)) and reports metrics marginally, per genetic group (ST), and on the unseen-lineage holdout (`models/eval_summary.json`). Only gentamicin has both classes in its gate-negative unseen-lineage fold; it holds up (balanced accuracy 0.91, AUROC 0.882 vs test-fold 0.875), and the other four are surfaced as too thin/single-class to assess — not hidden.
- **No leakage (re-asserted) + no vibes numbers** — the harness reuses `predictor.split.make_split` verbatim and re-invokes `no_leakage_check`; because that guard cannot see a wrong-but-internally-disjoint split, `ReproCheck` re-derives the committed metric sets (`predictor.train._metric_set` mirrored bit-for-bit) and asserts they equal the committed `metrics.json` (`committed_match=true`, max delta 0.0 for all five drugs). `scripts/run_eval.py` refuses to ship on divergence. Pinned by `tests/eval/` (unit parity + the e2e + the tamper tripwire).
- **Selective prediction + calibration (per slice)** — no-call rate + accuracy-on-called and uniform reliability bins at every granularity, computed on the served (gate-negative) population where the conformal set is actually applied at inference.
- **Honest limitations** — `MODEL_CARD.md` / `DATASHEET.md` state the n=130 / 67-group thinness, single-class folds (AUROC n/a for 3/5 drugs), gate-negative scope, conformal guarantee unavailable for 4/5, and mixed CLSI/EUCAST label noise — every figure traced to a committed artifact.
